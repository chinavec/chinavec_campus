...
<body>
    ...
    <div id="container" class="clearfix">
        ...
        <div id="container_right" class="right" style="text-align:center">
			...
			<form id="buttonInput" action="materialadd.php" method="get">
			 <table >
				   <tr>
           ...
           <form name="input" action="materialdel.php" method="POST" >
  ...
  $page_num =10;//每页记录数为10
        if (!isset($_GET['page_no']))//page_no 空
         ...
         $sql="SELECT * from `material` where `title` like '%".$keyword."%' or `abstract` like '%".$keyword."%' LIMIT $start_num, $page_num";
...
while ($result_row = mysql_fetch_assoc($query)) {
	...
	switch($result_row['material_type_id'])
                         {
						case 1:
                            $Type= "视频素材";
                            break;
...
<div>
	<tr class="distr" >
			<td width="10%" align="center" valign="middle" class="font-hui">{$result_row['id']}</td>
                        <td width="18%" align="center" valign="middle" class="font-hui">{$result_row['title']}</td>
                        <td width="14%" align="center" valign="middle" class="font-hui">$abstract</td>
						...
						<td width="22%" align="center" valign="middle"><a href="materialedit.php?id={$result_row['id']}">编辑</a>
...
TEXT;
  ...
  <span id="jilu1">显示<?php echo $nums; ?>条记录</span>
    <span id="jilu2">
        <?php
		$sql1 = "SELECT * from `material` where `title` like '%".$keyword."%' or `abstract` like '%".$keyword."%'";
		$result1 = mysql_query($sql1);
		$numss = mysql_num_rows($result1);
		$page = ceil($numss/$page_num);
            if ($page_no > 1) {
                    echo "<a href ='searchm.php?page_no=".($page_no-1)."'>上一页</a>&nbsp;&nbsp;&nbsp;";
                }else{
                    echo '<span>上一页</span>&nbsp;&nbsp;&nbsp;';
                }
                echo '<strong>第'.$page_no.'页/共'.$page.'页</strong>';
                if ($nums == $page_num) {
                    echo "&nbsp;&nbsp;&nbsp;<a href ='searchm.php?page_no=".($page_no+1)."'>下一页</a>";
                }else{
                    echo '&nbsp;&nbsp;&nbsp;<span>下一页</span>';
                }
        ?>
