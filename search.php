			...
			<form id="buttonInput" action="newsadd.php" method="get">
			 <table >
			     ...
			     <td> <input class="btn btn-success"  type="button" value="全选/全不选" id="select" onclick = "selectInvert('ID_Dele[]');"/></td>
			      <td> <input class="btn btn-primary" type="submit" value="新增" id="unSelect" /></td>
				  </tr>
			 </table> 
          </form>		
...
$sql="SELECT * from `college` where `title` like '%".$keyword."%' or `content` like '%".$keyword."%' LIMIT $start_num, $page_num";
...
while ($result_row = mysql_fetch_assoc($query)) {
	$title = mb_substr($result_row['title'],0,10,'utf-8')."....";
	$str = mb_substr($result_row['content'],0,10,'utf-8')."....";
...
TEXT;
  ...
  <span id="jilu1">显示<?php echo $nums; ?>条记录</span>
    <span id="jilu2">
        <?php
		$sql1 = "SELECT * from `college` where `title` like '%".$keyword."%' or `content` like '%".$keyword."%'";
