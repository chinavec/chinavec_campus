...
<body>
  ...
  $page_num =8;//每页记录数为10
        if (!isset($_GET['page_no']))//page_no 空
          ...
          $sql = "SELECT * from `material` limit $start_num, $page_num"; 
	...
	<tr class="distr" >
			<td width="10%" align="center" valign="middle" class="font-hui">{$result_row['id']}</td>
						...
						<td width="22%" align="center" valign="middle"><a href="materialedit.php?id={$result_row['id']}">编辑</a>
                         <a href="materialdel.php?id={$result_row['id']}" onclick="if(confirm('确实要删除此条记录吗？')) return submit();else return false;">&nbsp;&nbsp;删除</a>
  ...
  <span id="jilu1">显示<?php echo $nums; ?>条记录</span>
    <span id="jilu2">
		...
		$page = ceil($numss/$page_num);
            if ($page_no > 1) {
                    echo "<a href ='material.php?page_no=".($page_no-1)."'>上一页</a>&nbsp;&nbsp;&nbsp;";
                }else{
                    echo '<span>上一页</span>&nbsp;&nbsp;&nbsp;';
                }
                echo '<strong>第'.$page_no.'页/共'.$page.'页</strong>';
                if ($nums == $page_num) {
                    echo "&nbsp;&nbsp;&nbsp;<a href ='material.php?page_no=".($page_no+1)."'>下一页</a>";
                }else{
                    echo '&nbsp;&nbsp;&nbsp;<span>下一页</span>';
                }
        ?>
